export function createImageRenderer({
    ...
    function createMediaElement({ url, mediaType = 'image', seed, meta = {} }) {
        const element = document.createElement(mediaType === 'video' ? 'video' : 'img');
        if (mediaType === 'video') {
            element.controls = true;
            element.preload = 'metadata';
            element.playsInline = true;
            element.setAttribute('aria-label', 'AI 生成视频');
        } else {
            element.alt = 'Generated by AI';
        }
        element.src = url;
        element.style.height = 'auto';
        element.style.borderRadius = '4px';
        element.style.cursor = 'pointer';
        element._aiGenMeta = { seed, mediaType, ...meta };
        bindTooltip(element);
        element.addEventListener('dblclick', () => window.open(url, '_blank'));
        return element;
    }
    ...
    function waitForImageReady(img, timeoutMs = 1800) {
        if (img.tagName === 'VIDEO' && img.readyState >= 1) return Promise.resolve();
        ...
        const loadPromise = new Promise(resolve => {
            img.addEventListener(img.tagName === 'VIDEO' ? 'loadedmetadata' : 'load', resolve, { once: true });
    ...
    async function displayImageGrid(anchorElement, images) {
        ...
        const displayWidth = await getValue('comfyui_display_width');
        const imageElements = [];
        for (const { imageUrl, mediaType, seed } of images) {
            const img = createMediaElement({ url: imageUrl, mediaType, seed });
            img.style.width = displayWidth > 0 ? 'auto' : '100%';
            img.style.maxWidth = displayWidth > 0 ? `${displayWidth}px` : '100%';
    ...
    async function displayImage(anchorElement, imageDataOrId) {
        ...
        if (typeof imageDataOrId === 'string') {
            if (imageDataOrId.startsWith('http') ||
                ...
                mediaUrl = imageDataOrId;
                mediaType = /\.(mp4|webm|mov)(?:[?#]|$)/i.test(imageDataOrId) ? 'video' : 'image';
            } else {
                try {
                    ...
                    if (cached?.blob) {
                        ...
                        mediaUrl = objectUrl;
                        mediaType = cached.mediaType || (cached.blob.type.startsWith('video/') ? 'video' : 'image');
                        mediaMeta = { prompt: cached.prompt, ...cached.metadata };
                    } else {
                        throw new Error('缓存图片未找到');
                    }
                } catch (error) {
                    logger.error('[AI Gen] 加载缓存图片失败:', error);
                    mediaUrl = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg"><text x="10" y="20" fill="red">图片丢失</text></svg>';
                }
            }
        } else if (imageDataOrId?.url) {
            mediaUrl = imageDataOrId.url;
            mediaType = imageDataOrId.mediaType || 'image';
            mediaMeta = imageDataOrId.metadata || {};
        }

        const img = createMediaElement({ url: mediaUrl, mediaType, meta: mediaMeta });
