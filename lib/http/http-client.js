            ...
            const requestOptions = {
                ...
                onload: (response) => {
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        const responseDetail = options.redactErrorResponse
                            ? ''
                            : `. Response: ${response.responseText.substring(0, 100)}`;
                        reject(new Error(`API错误: ${response.status} ${response.statusText || ''}${responseDetail}`));
                    }
                },
                onerror: (error) => {
                ...
                redactUrlInLogs: !!options.redactUrlInLogs,
